...
function cleanUnicode(obj) {
  return JSON.parse(
    JSON.stringify(obj).normalize("NFKC")
      .replace(/[\u2028\u2029]/g, '')
      .replace(/\u00A0/g, ' ')
  );
}
...
export default async function updateGallery() {
  ...
  let previous = await kvGetGallery('gallery:json');
  if (!Array.isArray(previous)) {
    console.warn("⚠️ Warning: gallery:json is not an array, falling back to empty array.");
    previous = [];
  }
  ...
  const cleaned = cleanUnicode(gallery);
  console.log("🧪 Type of cleaned:", typeof cleaned);
  console.log("🧪 First item:", cleaned[0]);
  if (!Array.isArray(cleaned)) {
    console.error("❌ cleanUnicode result is not an array! Aborting save.");
    return 0;
  }
  await kvSetGallery('gallery:json', cleaned);
  console.log(`✅ gallery:json saved with ${gallery.length} entries`);

  if (previous.length) {
    const timestamp = Date.now();
    const backupKey = `gallery:backup:${timestamp}`;
    await kvSetGallery(backupKey, previous);
