...
export async function kvGetGallery(key) {
  ...
  try {
    ...
    // ✅ Unwrap { result: [...] } or { result: "[...]" }
    if (result && result.result) {
      if (Array.isArray(result.result)) {
        console.warn(`⚠️ ${key} was wrapped in { result: [...] }. Unwrapping.`);
      ...
      if (typeof result.result === 'string') {
        try {
          ...
          if (Array.isArray(parsed)) {
            console.warn(`⚠️ ${key} had result as stringified array. Parsed and unwrapped.`);
            return parsed;
          }
        } catch (e) {
          console.error(`❌ Failed to parse stringified result:`, e.message);
        }
      }
    }
    ...
    // ✅ Fully stringified array or wrapped again
    if (typeof result === 'string') {
      const once = JSON.parse(result);
      if (Array.isArray(once)) return once;
      if (once && Array.isArray(once.result)) {
        console.warn(`⚠️ ${key} was double-wrapped with { result: [...] }. Unwrapping.`);
        return once.result;
      }
    }

    // ✅ Attempt recovery from objectified array (e.g. {"0": {...}, "1": {...}})
    if (result && typeof result === 'object') {
      const maybeArray = Object.values(result);
      if (maybeArray.every(p => typeof p === 'object' && p !== null)) {
        console.warn(`⚠️ ${key} is an objectified array. Rehydrating.`);
        return maybeArray;
      }
    }
