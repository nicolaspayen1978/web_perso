...
async function updateGallery() {
  ...
  for (const file of validFiles) {
    ...
    if (format === 'L') editions.L = { total: EDITION_RULES.L, sold: 0 };
    if (format === 'XL') editions.XL = { total: EDITION_RULES.XL, sold: 0 };
    ...
    gallery.push({
      ...
      print_description: [
        ...
        `Gallery edition (L): ${sizeOptions.L.long}×${sizeOptions.L.short}cm — ${priceDetails.L}`,
        `Collector edition (XL): ${sizeOptions.XL.long}×${sizeOptions.XL.short}cm — ${priceDetails.XL}`,
