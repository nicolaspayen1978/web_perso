...
export async function kvGetGallery(key) {
  ...
  try {
    ...
    // ✅ Case 2a: { result: "[...]" } (stringified array)
    if (typeof result?.result === 'string') {
      try {
        ...
        // ✅ Case 2c: { result: "{\"result\":[...]}" }
        if (parsed && Array.isArray(parsed.result)) {
          console.warn(`✅ ${key} had result as stringified { result: [...] }. Double-unwrapped.`);
          return parsed.result;
        }

        // ✅ Case 2d: objectified array inside result
        if (parsed && typeof parsed === 'object') {
          const maybeArray = Object.values(parsed);
          if (maybeArray.every(p => typeof p === 'object' && p !== null)) {
            console.warn(`✅ ${key} had result as stringified objectified array. Rehydrated.`);
            return maybeArray;
          }
        }
      } catch (e) {
